...
void imgui_display() {
   if(imgui_shouldRenderSplineControls)
   {
      ...
      ImGui::Checkbox("Allow coordinate editing on GUI", &imgui_allowCoordinateEditingOnGUI);
      ...
      if(draggingPoint) {
         ImGui::Text("DraggingPoint: (%lf, %lf)", draggingPoint[0], draggingPoint[1]);
      }
      else {
...
void drawControlPoints() {
   ...
   if(!shouldDrawControlPoints) return;
   glPointSize(5.0);
